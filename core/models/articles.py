...
class FullArticles:
    ...
    def _generate_full_article(self):
        ...
        for article in self._articles:
            print(f"[+] Article: {article['urlTitle']}")
            article = self._format_date(article)
            hashed_article = hashlib.sha256(str(article).encode('utf-8')).hexdigest()
            full_article_text = self._scrape_article_full_text(article, hashed_article)
    ...
    def _scrape_article_full_text(self, article, hashed_article):
